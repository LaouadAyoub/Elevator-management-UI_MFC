...
bool CAscenseur::appel(int eta_dem)
{
	if (enfonction == true)
	{
		...
		{
			MessageBox(NULL, L"had l'étage makaynch asat 3iet lcharika", L"Ascenseur", MB_OK);
			enfonction = false;

		}
		else if (eta_dem == netage)
		{
			MessageBox(NULL, L"rak deja fhad l'étage asat", L"Ascenseur", MB_OK);
	...
	{
		cout << endl << enfonction;
		MessageBox(NULL, L"khasser", L"Ascenseur", MB_OK);
	}
	return enfonction;
}
